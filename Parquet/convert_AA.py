...
dir_name = 'Updated_ATagger_AA_HLT_Ng2_{}GeV_{}'.format(mass, date)
#dir_name = 'Ng2_Test_ATagger_AA_HLT_Iso_Ng2_{}GeV_{}'.format(mass, date)
...
parser.add_argument('-i', '--infile', default=['test_{}_AA_{}GeV'.format(date, mass)], nargs='+', type=str, help='Input root file.')
#parser.add_argument('-o', '--outdir', default='./', type=str, help='Output pq file dir.')
parser.add_argument('-o', '--outdir', default='/eos/uscms/store/user/kyungmip/CONDOR_TEST/', type=str, help='Output pq file dir.')
parser.add_argument('-d', '--decay', default='HToAA_MA-{}GeV'.format(mass), type=str, help='Decay name.')
...
if 'test' not in rhTreeStr[0]:
    ...
    outlist.remove(path+'/output_10.root')
...
for iEvt in range(iStart,iEnd):
    ...
    for i in range(nPhoEvt):
        #print(rhTree.PhoObj_genMatchedPdgId[i])
